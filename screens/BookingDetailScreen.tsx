...
export const BookingDetailScreen: React.FC<Props> = ({
  navigateTo,
  booking,
}) => {
  const steps = [
    ...
    <div className="flex flex-col min-h-screen bg-gray-50 dark:bg-[#121212] font-display text-onyx dark:text-white pb-safe">
      <header className="bg-white dark:bg-[#1a1a1a] p-4 flex items-center gap-4 shadow-sm z-20 sticky top-0">
        ...
        <div className="flex-1 text-right">
          <button className="text-xs font-bold text-primary uppercase border border-primary/30 px-3 py-1 rounded-full">
            Help
          </button>
        </div>
      </header>
          ...
          {steps.map((step, idx) => {
            ...
            return (
              <div key={idx} className="relative flex items-start gap-4">
                <div
                  className={`z-10 h-6 w-6 rounded-full border-4 flex items-center justify-center ${isCompleted ? "bg-primary border-primary" : "bg-white dark:bg-[#1a1a1a] border-gray-300 dark:border-gray-600"}`}
                >
                  {isCompleted && (
                ...
                <div className={`flex-1 ${!isCompleted && "opacity-50"}`}>
                  <h4
                    className={`text-sm font-bold ${isCurrent ? "text-primary" : "text-onyx dark:text-white"}`}
                  >
                    {step.label}
                  </h4>
                  {step.time && (
