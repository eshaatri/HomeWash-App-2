...
export const ServiceSelectionScreen: React.FC<NavigationProps> = ({
  navigateTo,
  selectedCategory,
        ...
        await Promise.all(
          servicesToResolve.map(async (s) => {
            try {
              ...
              newPrices[s.id] = result.finalPrice;
            } catch (e) {
              console.error(`Failed to resolve price for ${s.id}:`, e);
              newPrices[s.id] = s.price; // Fallback to base price
            }
          }),
        );

        setResolvedPrices((prev) => ({ ...prev, ...newPrices }));
      } finally {
      ...
      <main className="flex-1 pb-32">
        ...
        <div className="px-5 py-6 border-b border-gray-100 dark:border-white/5">
          ...
          <div className="flex items-center gap-2">
            <div className="flex items-center justify-center h-5 w-5 rounded-full bg-black dark:bg-white text-white dark:text-black">
              <span className="material-symbols-outlined text-[12px] font-bold fill-1">
                star
              </span>
            </div>
            ...
            {categoryMeta.sections[0].items.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  <span
                    className={`text-[10px] font-bold text-center leading-tight transition-colors ${isActive ? "text-onyx dark:text-white" : "text-gray-400"}`}
                  >
                    {item.name}
                  </span>
                </button>
        ...
        <div className="px-5 py-6">
          <h2 className="text-2xl font-black tracking-tight">
            {
              categoryMeta.sections[0].items.find((i) => i.id === activeSubId)
                ?.name
            }
          </h2>
        </div>
          ...
          {filteredServices.length > 0 ? (
            filteredServices.map((service: ExtendedService) => {
              ...
              return (
                <div
                  key={service.id}
                  className="relative bg-white dark:bg-[#111] rounded-2xl p-5 border border-gray-100 dark:border-white/5 shadow-sm hover:shadow-md transition-all"
                >
                  <div className="flex gap-4">
                    <div className="flex-1">
                      ...
                      <h3
                        ...
                        onClick={() =>
                          isConfigurable && handleServiceClick(service)
                        }
                      >
                        {service.title}
                      </h3>
                      ...
                      <div className="flex items-center gap-1 mb-3">
                        <span className="text-base font-black text-onyx dark:text-white">
                          Starts at ₹
                          {resolvedPrices[service.id] || service.price}
                        </span>
                        {resolvedPrices[service.id] &&
                          resolvedPrices[service.id] !== service.price && (
                            <span className="text-[10px] bg-blue-100 text-blue-600 px-1.5 rounded font-bold uppercase tracking-wider">
                              Area Price
                            </span>
                          )}
                      </div>
            ...
            <div className="flex flex-col items-center justify-center py-20 opacity-30">
              ...
              <p className="font-bold mt-4">
                No packages found for this sub-category
              </p>
            </div>
          )}
        </div>
          ...
          <div className="space-y-6">
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400 mt-1">
                  Background checked and trained experts only.
                </p>
              </div>
            </div>
            <div className="flex items-start gap-4">
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400 mt-1">
                  Advanced machine-driven scrubbing technology.
                </p>
              </div>
            </div>
          </div>
