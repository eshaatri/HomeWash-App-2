...
export const HomeScreen: React.FC<NavigationProps> = (props) => {
  ...
  return (
    <div className="relative flex h-full min-h-screen w-full flex-col overflow-x-hidden pb-24 bg-onyx">
      {/* Top App Bar */}
      <header className="sticky top-0 z-30 flex items-center justify-between bg-onyx/90 px-6 py-4 backdrop-blur-md border-b border-white/5">
        <div className="flex items-center gap-2 text-alabaster/80" onClick={() => navigateTo(AppScreen.ADDRESSES)}>
          <span className="material-symbols-outlined text-primary" style={{ fontSize: '20px' }}>location_on</span>
          <span className="text-xs font-medium tracking-widest uppercase cursor-pointer hover:text-white">MUMBAI • Bandra West</span>
        </div>
        <button 
          onClick={togglePremium}
          className={`relative flex h-10 w-10 items-center justify-center overflow-hidden rounded-full border border-white/10 bg-onyx-light transition-transform active:scale-95 ${!isPremium ? 'grayscale' : ''}`}
        >
          <img 
            alt="Profile" 
      ...
      <div className="px-6 pt-6 pb-2">
        <h1 className="flex flex-col text-[4rem] font-black leading-[0.85] tracking-tighter text-alabaster">
          <span>HOME</span>
          <span className="text-white/20">WASH</span>
        </h1>
        {!isPremium && (
      ...
      <div className="w-full overflow-x-auto no-scrollbar pb-6 pl-6">
        <div className="flex gap-4 pr-6">
          <button 
            ...
            className="whitespace-nowrap rounded-full border border-white/10 bg-onyx-light px-5 py-2.5 text-sm font-medium text-white transition-colors hover:bg-white hover:text-onyx active:scale-95"
          >
            Book Now
          </button>
          <button className="whitespace-nowrap rounded-full border border-white/10 bg-onyx-light px-5 py-2.5 text-sm font-medium text-white transition-colors hover:bg-white hover:text-onyx active:scale-95">
            Recent Orders
          </button>
          <button 
      ...
      <section className="px-6">
        <div className="flex items-center justify-between pb-4 pt-2">
          <h3 className="text-lg font-bold tracking-tight text-white">Concierge Services</h3>
          <span 
            className="text-xs font-medium text-white/40 cursor-pointer hover:text-white"
            onClick={() => navigateTo(AppScreen.SERVICE_SELECTION)}
          >
            View All
          </span>
        ...
        <div className="grid grid-cols-2 gap-3">
          {[
            ...
            <div 
              key={service.title} 
              onClick={() => navigateTo(AppScreen.SERVICE_SELECTION)}
              className="group flex cursor-pointer flex-col justify-between gap-4 rounded-xl border border-white/5 bg-onyx-light p-4 transition-all hover:border-primary/50 hover:bg-[#1a1a1a] active:scale-[0.98]"
            >
              <div className="flex h-10 w-10 items-center justify-center rounded-full bg-white/5 text-alabaster group-hover:bg-primary group-hover:text-onyx transition-colors">
                <span className="material-symbols-outlined" style={{ fontSize: '20px' }}>{service.icon}</span>
              </div>
              <div>
                <h4 className="text-base font-bold leading-tight text-alabaster">{service.title}</h4>
                <p className="mt-1 text-[10px] uppercase tracking-wider text-white/40">{service.price}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <section className="mt-8 px-6">
        <div 
          onClick={() => navigateTo(AppScreen.SERVICE_SELECTION)}
          className="relative overflow-hidden rounded-xl bg-[#e5e5e5] p-6 text-onyx shadow-lg cursor-pointer"
        >
