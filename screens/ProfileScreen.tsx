...
export const ProfileScreen: React.FC<NavigationProps> = (props) => {
  ...
  useEffect(() => {
    if (!isEditing && user) {
      setEditName(user.name === "New User" ? "" : user.name);
      ...
      <main className="flex-1 px-6 py-6 space-y-6">
        ...
        <div className="flex flex-col items-center justify-center space-y-4">
          <div className="relative">
            <div className="h-24 w-24 rounded-full flex items-center justify-center bg-primary/20 text-primary border-4 border-white dark:border-[#1a1a1a] shadow-lg">
            ...
            {isEditing ? (
              <div className="space-y-3 animate-in fade-in zoom-in-95 duration-200">
                <div className="space-y-1">
                  <label className="text-[10px] font-bold uppercase tracking-widest text-gray-400 text-left block ml-1">
                    Full Name
                  </label>
                  <input
                    type="text"
                    ...
                    className="w-full h-10 bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl px-4 text-sm font-bold focus:outline-none focus:ring-2 focus:ring-primary/50"
                  />
                </div>
                <div className="space-y-1">
                  <label className="text-[10px] font-bold uppercase tracking-widest text-gray-400 text-left block ml-1">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    disabled={isUpdating}
                    className="flex-1 h-10 bg-primary text-black font-bold rounded-xl active:scale-95 transition-all text-xs uppercase tracking-wider"
                  >
                    {isUpdating ? "Saving..." : "Save Profile"}
                  </button>
                  <button
                    onClick={() => setIsEditing(false)}
                    className="h-10 px-4 bg-gray-100 dark:bg-white/5 text-gray-500 font-bold rounded-xl active:scale-95 transition-all text-xs uppercase tracking-wider"
                  >
                    Cancel
                  </button>
                </div>
              ...
              <>
                <div className="flex items-center justify-center gap-2 group">
                  <h2 className="text-2xl font-black text-onyx dark:text-white capitalize">
                    {user?.name === "New User"
                      ? "Complete Profile"
                  ...
                  <button
                    ...
                    className="h-8 w-8 rounded-full bg-gray-100 dark:bg-white/5 flex items-center justify-center text-gray-400 hover:text-primary transition-colors"
                  >
                ...
                <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-1 opacity-60">
        ...
        <div className="bg-gradient-to-br from-onyx to-[#2a2a2a] dark:from-[#1a1a1a] dark:to-[#000] rounded-2xl p-6 text-white shadow-xl relative overflow-hidden">
          ...
          <span className="material-symbols-outlined absolute -bottom-8 -right-8 text-[120px] opacity-[0.05] rotate-[-15deg]">
        ...
        <div className="space-y-3">
          ...
          <div className="bg-white dark:bg-[#1a1a1a] rounded-xl border border-gray-100 dark:border-white/5 overflow-hidden">
            ...
            >
              ...
              <div className="text-left flex-1">
                <h4 className="font-bold text-onyx dark:text-white">
                  My Bookings
                </h4>
                <p className="text-xs text-gray-500">
                  Track active & past jobs
                </p>
              </div>
          ...
          <div className="bg-white dark:bg-[#1a1a1a] rounded-xl border border-gray-100 dark:border-white/5 overflow-hidden">
            <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-white/5">
              ...
              >
                <div
                  className={`absolute top-1 left-1 bg-white w-5 h-5 rounded-full shadow-sm transition-transform ${isDarkMode ? "translate-x-5" : "translate-x-0"}`}
                ></div>
              </button>
            </div>
            ...
            <div className="flex items-center justify-between p-4">
              ...
              >
                <div
                  className={`absolute top-1 left-1 bg-white w-5 h-5 rounded-full shadow-sm transition-transform ${isPremium ? "translate-x-5" : "translate-x-0"}`}
                ></div>
              </button>
            </div>
          </div>
          ...
          <button
            onClick={logout}
            className="w-full bg-red-50 dark:bg-red-900/10 text-red-600 font-bold p-4 rounded-xl border border-red-100 dark:border-red-900/20 active:scale-[0.98] transition-transform"
          >
            Log Out
          </button>
        </div>
